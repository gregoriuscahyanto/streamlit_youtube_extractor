...
def test_extractor_constrains_standard_rpm_and_persists_metadata():
    txt = _read("app.py")
    assert "from app_tabs.audio_sweep import constrain_rpm_to_gear_bands" in txt
    assert "rpm, gear_limit_meta = constrain_rpm_to_gear_bands(t_video, rpm, gear_band_cfg)" in txt
